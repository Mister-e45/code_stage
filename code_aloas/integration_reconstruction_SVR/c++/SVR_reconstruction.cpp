...
#include <SVR_reconstruction.h>


#define STOPFIRE ((ReconstructionSVR*) treatmentInstance->userdata)->stopFire
/*
 ...
 */
//#define SVR_RECONTRUCTION_DATAS_FIELD1 ((TmtSVR_recontructionData*)treatmentInstance->userdata)->field1
//#define SVR_RECONTRUCTION_DATAS_FIELD2 ((TmtSVR_recontructionData*)treatmentInstance->userdata)->field2
//#define SVR_RECONTRUCTION_DATAS_FIELD3 ((TmtSVR_recontructionData*)treatmentInstance->userdata)->field3
//#define SVR_RECONTRUCTION_DATAS_FIELD4 ((TmtSVR_recontructionData*)treatmentInstance->userdata)->field4
////etc...
/* 
...
void ALOAS_TMT_SVR_RECONSTRUCTION_API SVR_reconstruction_buildTemplate(void* templatetmt) {
    ...
    TAlsTemplatePort* portDimensionCode = als_tmttpt_createParameterPort(templatetmt, L"Dimension du Code");
    TAlsTemplatePort* portNombreColonneEchantillon = als_tmttpt_createParameterPort(templatetmt, L"ns");
    TAlsTemplatePort* portNombreMotsEchantillon = als_tmttpt_createParameterPort(templatetmt, L"ms");
    TAlsTemplatePort* portD_s = als_tmttpt_createParameterPort(templatetmt, L"d_s");
    TAlsTemplatePort* portW_thres = als_tmttpt_createParameterPort(templatetmt, L"w_thres");
    TAlsTemplatePort* portN_iter = als_tmttpt_createParameterPort(templatetmt, L"N_iter");
    TAlsTemplatePort* portNombreLimiteEchantillonnage = als_tmttpt_createParameterPort(templatetmt, L"LimIter");
    TAlsTemplatePort* portNombreMotsRecus = als_tmttpt_createParameterPort(templatetmt, L"Nombre de Mots Recus");
    TAlsTemplatePort* portTailleBlocsCirculant = als_tmttpt_createParameterPort(templatetmt, L"Taille des Blocs Circulants");
    ...
    TAlsTemplatePort* outputPortK = als_tmttpt_createOutputPort(templatetmt, L"Y2");
    TAlsTemplatePort* outputPortN = als_tmttpt_createOutputPort(templatetmt, L"Y3");


    als_tmttpt_setTypeEquals(portLongueurCode, &IntType);
    als_tmttpt_setTypeEquals(portDimensionCode, &IntType);
    als_tmttpt_setTypeEquals(portNombreColonneEchantillon, &IntType);
    als_tmttpt_setTypeEquals(portNombreMotsEchantillon, &IntType);
    als_tmttpt_setTypeEquals(portD_s, &IntType);
    als_tmttpt_setTypeEquals(portW_thres, &IntType);
    als_tmttpt_setTypeEquals(portN_iter, &IntType);
    als_tmttpt_setTypeEquals(portNombreLimiteEchantillonnage, &IntType);
    als_tmttpt_setTypeEquals(portNombreMotsRecus, &IntType);
    als_tmttpt_setTypeEquals(portTailleBlocsCirculant, &IntType);

    als_tmttpt_setTypeAtMost(inputPortMotsRecus, &BitType);

    als_tmttpt_setTypeAtLeast(outputPort, &BitType);
    als_tmttpt_setTypeAtLeast(outputPortK, &IntType);
    als_tmttpt_setTypeAtLeast(outputPortN, &IntType);
...
void ALOAS_TMT_SVR_RECONSTRUCTION_API SVR_reconstruction_preinitialize(TAlsTIStruct* treatmentInstance) {
    treatmentInstance->userdata = malloc(sizeof (ReconstructionSVR));
    STOPFIRE = false;
    /** \n <b> Structure de la fonction </b> : \li Début de la fonction */
     ...
     * Exemple :
     */
    // treatmentInstance->userdata = new TmtSVR_recontructionData;
    // // Initialisation des champs de la structure
    // SVR_RECONTRUCTION_DATAS_FIELD1 = 0;
    // SVR_RECONTRUCTION_DATAS_FIELD2 = 0;
    // als_data_initializeDoubleBloc(&SVR_RECONTRUCTION_DATAS_FIELD3);
    // als_data_initializeToken(&SVR_RECONTRUCTION_DATAS_FIELD4, ADT_UNKNOWN);
...
void ALOAS_TMT_SVR_RECONSTRUCTION_API SVR_reconstruction_fire(TAlsTIStruct* treatmentInstance) {
    ...
    TAlsReceiver *port_k=als_ifc_getParamReceiver(treatmentInstance,1);
    TAlsReceiver *port_ns=als_ifc_getParamReceiver(treatmentInstance,2);
    TAlsReceiver *port_ms=als_ifc_getParamReceiver(treatmentInstance,3);
    TAlsReceiver *port_ds=als_ifc_getParamReceiver(treatmentInstance,4);
    TAlsReceiver *port_wthres=als_ifc_getParamReceiver(treatmentInstance,5);
    TAlsReceiver *port_Niter=als_ifc_getParamReceiver(treatmentInstance,6);
    TAlsReceiver *port_itLim=als_ifc_getParamReceiver(treatmentInstance,7);
    TAlsReceiver *port_N_MotsRecus=als_ifc_getParamReceiver(treatmentInstance,8);
    TAlsReceiver *port_tailleBlocsCirculant=als_ifc_getParamReceiver(treatmentInstance,9);
    ...
    TAlsReceiver *port_sortieK=als_ifc_getOutputReceiver(treatmentInstance, 1);
    TAlsReceiver *port_sortieN=als_ifc_getOutputReceiver(treatmentInstance, 2);

    IntBlock n_param, k_param, ns_param, ms_param, ds_param, wthres_param, Niter_param, itLim_param, N_recus_param,circulant_param;
    
    als_data_initializeIntBlock(&n_param);
    als_data_initializeIntBlock(&k_param);
    als_data_initializeIntBlock(&ns_param);
    als_data_initializeIntBlock(&ms_param);
    als_data_initializeIntBlock(&ds_param);
    als_data_initializeIntBlock(&wthres_param);
    als_data_initializeIntBlock(&Niter_param);
    als_data_initializeIntBlock(&itLim_param);
    als_data_initializeIntBlock(&N_recus_param);
    als_data_initializeIntBlock(&circulant_param);
    ...
    else if (als_ifc_getSampleCount(port_k) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Dimension du Code> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_ns) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <ns> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_ms) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <ms> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_ds) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <d_s> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_wthres) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <w_thres> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_Niter) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <N_iter> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_itLim) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <LimIter> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_N_MotsRecus) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <LimIter> n'a pas été récupéré.");
    }
    else if (als_ifc_getSampleCount(port_tailleBlocsCirculant) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <LimIter> n'a pas été récupéré.");
    }
    else{
        ...
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 2, &ns_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <ns> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 3, &ms_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <ms> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 4, &ds_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <d_s> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 5, &wthres_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <w_thres> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 6, &Niter_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <N_iter> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 7, &itLim_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <LimIter> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 8, &N_recus_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de Mots Recus> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 9, &circulant_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Taille des Blocs Circulants> n'a pas été récupéré.");
        }
    }

    int n,k,ns,ms,d_s,w_thres,N_iter,itLim, num_rec;
    ...
    ms=ms_param.ptr[0];
    d_s=ds_param.ptr[0];
    w_thres=wthres_param.ptr[0];
    N_iter=Niter_param.ptr[0];
    itLim=itLim_param.ptr[0];
    num_rec=N_recus_param.ptr[0];
    ...
    if (!STOPFIRE){
        ...
        als_ifc_setConsumptionRate(port_entree,n*num_rec);  

        if (als_ifc_getSampleCount(port_entree) >= n*num_rec  && !als_ifc_cancelled(treatmentInstance) ){
            als_data_initializeToken(&bitEntree,ADT_UNKNOWN);
            als_ifc_readTokenBySamples(port_entree,&bitEntree, n*num_rec );

            if (als_data_upgradeTokenType(&bitEntree, ADT_BIT) == ACR_SUCCESS && bitEntree.bitBlock.size>=(n*num_rec)){


                RawMatrix motsRecus=create_empty_rows_matrix(num_rec);
                motsRecus.ncols=n;
                ...
                RawMatrix recons= reconstruction_SVR(motsRecus.rows,num_rec,n,k,ns,ms,d_s,w_thres,N_iter,num_rec,itLim);

                als_data_initializeBitBlock(&out);
                als_data_createBitBlock(&out,recons.nrows*recons.ncols);

                als_data_initializeIntBlock(&K);
                als_data_createIntBlock(&K,1);

                als_data_initializeIntBlock(&N);
                als_data_createIntBlock(&N,1);

                for(unsigned int i=0;i<recons.nrows;++i){
                    for(unsigned int j=0;j<recons.ncols;++j){
                        als_data_setBit(&out,i*recons.ncols+j,rawmatrix_get_value(recons,i,j));
                    }
                }
                
                K.ptr[0]=recons.nrows;
                N.ptr[0]=recons.ncols;

                als_ifc_writeBitBlock(port_sortie, &out);
                als_data_releaseBitBlock(&out);

                als_ifc_writeIntBlock(port_sortieK, &K);
                als_data_releaseIntBlock(&K);

                als_ifc_writeIntBlock(port_sortieN, &N);
                als_data_releaseIntBlock(&N);

                destroy_rawmatrix(motsRecus);
                destroy_rawmatrix(recons);

            }
            else{
	            ((ReconstructionSVR*) treatmentInstance->userdata)->stopFire = true;
    ...
    als_data_releaseIntBlock(&ns_param);
    als_data_releaseIntBlock(&ms_param);
    als_data_releaseIntBlock(&ds_param);
    als_data_releaseIntBlock(&wthres_param);
    als_data_releaseIntBlock(&Niter_param);
    als_data_releaseIntBlock(&itLim_param);
    als_data_releaseIntBlock(&N_recus_param);
    /** \n <b> Structure de la fonction </b> : \li Début de la fonction */
     ...
     */
    // TmtSVR_recontructionData userdata = *((TmtSVR_recontructionData*) treatmentInstance->userdata);
    ...
    //  if (0 < als_ifc_getParameterBoolBlock(treatmentInstance, 0, &SVR_RECONTRUCTION_DATAS_FIELD3)) {
    //    assert(SVR_RECONTRUCTION_DATAS_FIELD3.size > 0);
    ...
    //     */
    //    outputComplexBlock.ptr_r[i] = SVR_RECONTRUCTION_DATAS_FIELD3.ptr[0];
...
free(treatmentInstance->userdata);
    ...
    //  if (SVR_RECONTRUCTION_DATAS_FIELD2 != 0) {
    //    free(SVR_RECONTRUCTION_DATAS_FIELD2);
    //  }
    //  // Libération des blocs
    //  als_data_releaseDoubleBlock(&SVR_RECONTRUCTION_DATAS_FIELD3);
    //  als_data_releaseToken(&SVR_RECONTRUCTION_DATAS_FIELD4);
