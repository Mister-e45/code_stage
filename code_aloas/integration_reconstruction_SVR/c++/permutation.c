...
unsigned int* give_random_distinct_integers(unsigned int sampleSize,unsigned int max){ // fisher-yatte algorithm
...
void random_distinct_elements_in_range(unsigned int sampleSize,unsigned int min,unsigned int max,unsigned int* array){ // fisher-yatte algorithm
    ...
    Permutation p=create_permutation(max-min); // gives a random sample in [min,max[
#ifdef DEBUG
if(sampleSize>max){
    ...
    for(unsigned int k=max-min;k>limit;--k){
        unsigned int j=rand()%k;
        swap_values_in_array(p.permutation,j,k-1);
    }
    for(unsigned int k=max-min;k>limit;--k){
        array[k-1-limit]=p.permutation[k-1]+min;
    }
    destroy_permutation(p);
}
