...
RawMatrix rawmatrix_one_on_diagonal(unsigned int nrows,unsigned int ncols){
    unsigned int min= (nrows>ncols)? ncols : nrows;
    RawMatrix out=create_rawmatrix(nrows,ncols);
    for(unsigned int k=0;k<min;++k){
        rawmatrix_write(out,k,k,1);
    }
    return out;
}
...
void append_rows_to_rawmatrix(RawMatrix* mat,List bitsList){
    if(bitsList.listSize!=0){
        Bits** temp=mat->rows;

        mat->rows=calloc(mat->nrows+bitsList.listSize,sizeof(Bits*));
        if(mat->rows==NULL){
            printf("could not append vectors to raw matrix\n");
            mat->rows=temp;
        }
        else{
            for(unsigned int k=0;k<mat->nrows;++k){
            ...
            ListElement* element=bitsList.firstElement;
            for(unsigned int k=mat->nrows;k<mat->nrows+bitsList.listSize;++k){
                mat->rows[k]=(Bits*)element->value;
                element=next_ListElement(element);
            }
            free(temp);
            mat->nrows += bitsList.listSize;
        }
    }
}

void row_resize_rawmatrix(RawMatrix* mat,unsigned int newNrows){
    if(newNrows<mat->nrows){
        Bits** temp=mat->rows;

        mat->rows=calloc(newNrows,sizeof(Bits*));
        if(mat->rows==NULL){
            printf("could not append vectors to raw matrix\n");
            mat->rows=temp;
        }
        else{
            for(unsigned int k=0;k<newNrows;++k){
                mat->rows[k]=temp[k];
            }
            for(unsigned int k=newNrows;k<mat->nrows;++k){
                free(temp[k]);
            }
            free(temp);
        }
        ...
        if(mat->rows==NULL){
            ...
            for(unsigned int k=newNrows;k<mat->nrows;++k){
                Vector nullRow=create_vector(mat->ncols);
                mat->rows[k]=nullRow.bits;
            }
            free(temp);
        }
        mat->nrows=newNrows;
