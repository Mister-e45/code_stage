...
unsigned int* copy_unsignedint_array(unsigned int* array,unsigned int size){
    unsigned int* copy=calloc(size,sizeof(unsigned int));
...
void destroy_unsigned_int_array_list(void* pointerToList){
    destroy_List(pointerToList,&free);
    free(pointerToList);
}
