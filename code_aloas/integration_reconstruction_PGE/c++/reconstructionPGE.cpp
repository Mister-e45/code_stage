 ...
 *    complexe.
 */
#define STOPFIRE ((Reconstruction_PGE*) treatmentInstance->userdata)->stopFire

 extern "C"{

#include "PGE.h"
#include <reconstructionPGE.h>
 ...
 */
//#define RECONSTRUCTIONPGE_DATAS_FIELD1 ((TmtReconstructionPGEData*)treatmentInstance->userdata)->field1
//#define RECONSTRUCTIONPGE_DATAS_FIELD2 ((TmtReconstructionPGEData*)treatmentInstance->userdata)->field2
//#define RECONSTRUCTIONPGE_DATAS_FIELD3 ((TmtReconstructionPGEData*)treatmentInstance->userdata)->field3
//#define RECONSTRUCTIONPGE_DATAS_FIELD4 ((TmtReconstructionPGEData*)treatmentInstance->userdata)->field4
////etc...
/* 
...
void ALOAS_TMT_RECONSTRUCTIONPGE_API reconstructionPGE_buildTemplate(void* templatetmt) {
    ...
    TAlsTemplatePort* portObjectif = als_tmttpt_createParameterPort(templatetmt, L"Nombre de vecteurs libres du dual a trouver");
    ...
    TAlsTemplatePort* portLimiteBoucle = als_tmttpt_createParameterPort(templatetmt, L"Nombre maximal de tentatives d'application de PGE");
    TAlsTemplatePort* portNombreLimiteIterationRechercheVecteur = als_tmttpt_createParameterPort(templatetmt, L"Nombre maximal d'iterations de la recherche de vecteur");
    TAlsTemplatePort* portTailleBlocs = als_tmttpt_createParameterPort(templatetmt, L"Taille des blocs circulants");
    ...
    als_tmttpt_setTypeEquals(portObjectif, &IntType);
    ...
    als_tmttpt_setTypeEquals(portLimiteBoucle, &IntType);
    als_tmttpt_setTypeEquals(portNombreLimiteIterationRechercheVecteur, &IntType);
    als_tmttpt_setTypeEquals(portTailleBlocs, &IntType);
 ...
 */
 ...
 void ALOAS_TMT_RECONSTRUCTIONPGE_API reconstructionPGE_preinitialize(TAlsTIStruct* treatmentInstance) {
    treatmentInstance->userdata = malloc(sizeof (Reconstruction_PGE));
    STOPFIRE = false;
...
void ALOAS_TMT_RECONSTRUCTIONPGE_API reconstructionPGE_fire(TAlsTIStruct* treatmentInstance) {
    ...
    TAlsReceiver *port_objectif=als_ifc_getParamReceiver(treatmentInstance,1);
    TAlsReceiver *port_numRecus=als_ifc_getParamReceiver(treatmentInstance,2);
    TAlsReceiver *port_numUtilise=als_ifc_getParamReceiver(treatmentInstance,3);
    TAlsReceiver *port_tauxErreur=als_ifc_getParamReceiver(treatmentInstance,4);
    TAlsReceiver *port_t=als_ifc_getParamReceiver(treatmentInstance,5);
    TAlsReceiver *port_p=als_ifc_getParamReceiver(treatmentInstance,6);
    TAlsReceiver *port_maxIterationRecherche=als_ifc_getParamReceiver(treatmentInstance,7);
    TAlsReceiver *port_maxIterationProcedure=als_ifc_getParamReceiver(treatmentInstance,8);
    TAlsReceiver *port_tailleBlocs=als_ifc_getParamReceiver(treatmentInstance,9);
    ...
    IntBlock n_param, objectif_param, numRecus_param, numUtilises_param, t_param, p_param, maxIterPGE_param, maxIterProcedure_param, tailleBlocs_param;
    DoubleBlock tauxErreur_param;

    als_data_initializeIntBlock(&n_param);
    als_data_initializeIntBlock(&objectif_param);
    ...
    als_data_initializeIntBlock(&maxIterPGE_param);
    als_data_initializeIntBlock(&maxIterProcedure_param);
    als_data_initializeIntBlock(&tailleBlocs_param);
    ...
    else if (als_ifc_getSampleCount(port_objectif) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de vecteurs du dual a trouver> n'a pas été récupéré.");
    }
    ...
    else if (als_ifc_getSampleCount(port_maxIterationRecherche) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre maximal de tentatives d'application de PGE> n'a pas été récupéré.");
    }
    ...
    else if (als_ifc_getSampleCount(port_tailleBlocs) <= 0){
      STOPFIRE = true;
      als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre maximal d'iterations de recherche de vecteur> n'a pas été récupéré.");
    }
    else{
        ...
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 1, &objectif_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de vecteurs du dual a trouver> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 2, &numRecus_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de mots recus> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 3, &numUtilises_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de mots a utiliser dans PGE> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterDoubleBlock(treatmentInstance, 4, &tauxErreur_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Taux d'erreur> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 5, &t_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de Ligne a echelonner (t)> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 6, &p_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre de colonnes a collisionner (p)> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 7, &maxIterPGE_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre maximal de tentatives d'application de PGE> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 8, &maxIterProcedure_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Nombre maximal d'iterations de recherche de vecteur>> n'a pas été récupéré.");
        }
        if (!als_ifc_getParameterIntBlock(treatmentInstance, 9, &tailleBlocs_param)){
            STOPFIRE = true ; 
            als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Le parametre <Taille des blocs circulants> n'a pas été récupéré.");
        }
    }

    int n,rank,numRecus,numUtilises,t,p,maxIterPGE, maxIterProcedure,tailleBlocs;
    double tauxErreur;
    n=n_param.ptr[0];
    rank=objectif_param.ptr[0];
    numRecus=numRecus_param.ptr[0];
    numUtilises=numUtilises_param.ptr[0];
    tauxErreur=tauxErreur_param.ptr[0];
    t=t_param.ptr[0];
    p=p_param.ptr[0];
    maxIterPGE=maxIterPGE_param.ptr[0];
    maxIterProcedure=maxIterProcedure_param.ptr[0];
    tailleBlocs=tailleBlocs_param.ptr[0];
    if(numUtilises>numRecus || t>numUtilises || p>n-t){
        STOPFIRE = true ; 
        als_ifc_error(treatmentInstance, ALS_USER_ERROR, L"Paramètres non cohérents.");
    }
    ...
    if (!STOPFIRE){
        ...
        if (als_ifc_getSampleCount(port_entree) >= n*numRecus  && !als_ifc_cancelled(treatmentInstance) ){
            ...
            if (als_data_upgradeTokenType(&bitEntree, ADT_BIT) == ACR_SUCCESS && bitEntree.bitBlock.size>=(n*numRecus)){
                ...
                RawMatrix recons= QCLDPC_reconstruction_PGE(motsRecus.rows,numRecus,numUtilises,&tauxErreur,n,rank,tailleBlocs,t,p,maxIterPGE,maxIterProcedure);
	            ...
	            ((Reconstruction_PGE*) treatmentInstance->userdata)->stopFire = true;
    ...
    als_data_releaseIntBlock(&objectif_param);
    ...
    als_data_releaseIntBlock(&maxIterPGE_param);
    als_data_releaseIntBlock(&maxIterProcedure_param);
    als_data_releaseIntBlock(&tailleBlocs_param);
    /** \n <b> Structure de la fonction </b> : \li Début de la fonction */
     ...
     */
    // TmtReconstructionPGEData userdata = *((TmtReconstructionPGEData*) treatmentInstance->userdata);
    ...
    //  if (0 < als_ifc_getParameterBoolBlock(treatmentInstance, 0, &RECONSTRUCTIONPGE_DATAS_FIELD3)) {
    //    assert(RECONSTRUCTIONPGE_DATAS_FIELD3.size > 0);
    ...
    //     */
    //    outputComplexBlock.ptr_r[i] = RECONSTRUCTIONPGE_DATAS_FIELD3.ptr[0];
...
void ALOAS_TMT_RECONSTRUCTIONPGE_API reconstructionPGE_wrapup(TAlsTIStruct * treatmentInstance) {
    ...
    //  if (RECONSTRUCTIONPGE_DATAS_FIELD2 != 0) {
    //    free(RECONSTRUCTIONPGE_DATAS_FIELD2);
    //  }
    //  // Libération des blocs
    //  als_data_releaseDoubleBlock(&RECONSTRUCTIONPGE_DATAS_FIELD3);
    //  als_data_releaseToken(&RECONSTRUCTIONPGE_DATAS_FIELD4);
