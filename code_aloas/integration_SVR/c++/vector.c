...
Bit scalar_product(struct Vector v1,struct Vector v2){
    if(v1.size!= v2.size){
        ...
        int count=0; // pour compter le nombre de 1
...
void permute_vector(Permutation sigma,Vector* v){ // avoid use if you are using Vector struture as a temporary container for Bits
...
void vector_set_positions_to_one(Vector vector,unsigned int* positions,unsigned int positonsCount,unsigned int offset){
    for(unsigned int k=0;k<positonsCount;++k){
        put_coordinate(vector,positions[k]+offset,1);
    }
}